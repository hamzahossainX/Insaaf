...
async function main() {
  ...
  const supplier = await prisma.supplier.upsert({
    ...
    update: {},
    create: { id: "seed-supplier-1", name: "Bengal Gas Suppliers Ltd.", phone: "01800000001", address: "Tongi, Gazipur", notes: "Primary oxygen/nitrogen refill partner" },
  });
  const supplierProduct = products[0]; // Industrial Oxygen 7m3

  const existingSend = await prisma.supplierCylinderSend.findFirst({ where: { supplier_id: supplier.id } });
  if (!existingSend) {
    const sendQty = 10;
    const send = await prisma.supplierCylinderSend.create({
      data: { supplier_id: supplier.id, product_id: supplierProduct.id, quantity_sent: sendQty, note: "Empties sent for refill (seed)", user_id: admin.id },
    });
    const p = await prisma.product.findUniqueOrThrow({ where: { id: supplierProduct.id } });
    const newBal = p.current_stock_qty - sendQty;
    await prisma.stockLedgerEntry.create({ data: { product_id: supplierProduct.id, movement_type: StockMovementType.SUPPLIER_SEND_OUT, quantity: -sendQty, reference_type: "SUPPLIER_SEND", reference_id: send.id, resulting_balance: newBal } });
    await prisma.product.update({ where: { id: supplierProduct.id }, data: { current_stock_qty: newBal } });
    await prisma.supplierCylinderHold.upsert({
      where: { supplier_id_product_id: { supplier_id: supplier.id, product_id: supplierProduct.id } },
      update: { quantity_with_supplier: { increment: sendQty } },
      create: { supplier_id: supplier.id, product_id: supplierProduct.id, quantity_with_supplier: sendQty },
    });

    // Receipt: 6 of those come back refilled, plus 4 brand-new cylinders in the same delivery —
    // the exact "refill alongside new" scenario. Total charged 10,00,000 (10 lac), paid 0 now.
    const refillQty = 6;
    const newQty = 4;
    const receiptTotal = 1000000;
    const receipt = await prisma.supplierReceipt.create({
      data: {
        supplier_id: supplier.id, product_id: supplierProduct.id, refill_quantity: refillQty, new_quantity: newQty,
        total_amount: receiptTotal, paid_now_amount: 0, due_amount: receiptTotal, note: "Refill return + new cylinders in one delivery (seed)", user_id: admin.id,
      },
    });
    const p2 = await prisma.product.findUniqueOrThrow({ where: { id: supplierProduct.id } });
    const newBal2 = p2.current_stock_qty + refillQty + newQty;
    await prisma.stockLedgerEntry.create({ data: { product_id: supplierProduct.id, movement_type: StockMovementType.SUPPLIER_RECEIPT_IN, quantity: refillQty + newQty, reference_type: "SUPPLIER_RECEIPT", reference_id: receipt.id, resulting_balance: newBal2 } });
    await prisma.product.update({ where: { id: supplierProduct.id }, data: { current_stock_qty: newBal2 } });
    await prisma.supplierCylinderHold.update({
      where: { supplier_id_product_id: { supplier_id: supplier.id, product_id: supplierProduct.id } },
      data: { quantity_with_supplier: { decrement: refillQty } },
    });
    await prisma.supplier.update({ where: { id: supplier.id }, data: { current_payable_balance: { increment: receiptTotal } } });

    // Month-by-month payoff: 1 lac this month, matching the client's exact example.
    const payment1 = await prisma.supplierPayment.create({
      data: { supplier_id: supplier.id, amount: 100000, type: "INSTALLMENT", paid_from_account_id: bank.id, note: "First installment (seed)", user_id: admin.id },
    });
    await prisma.accountLedgerEntry.create({ data: { account_id: bank.id, type: "OUT", amount: 100000, reference_type: "SUPPLIER_PAYMENT", reference_id: payment1.id, resulting_balance: Number((await prisma.account.findUniqueOrThrow({ where: { id: bank.id } })).current_balance) - 100000 } });
    await prisma.account.update({ where: { id: bank.id }, data: { current_balance: { decrement: 100000 } } });
